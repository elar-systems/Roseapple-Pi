...
fix_partuuid() {
  ...
  sed -i "s/${OLD_DISKID}/${DISKID}/" /boot/uenv.txt
}
...
sed -i 's| init=/root/init_resize.sh||' /boot/uenv.txt
if ! grep -q splash /boot/uenv.txt; then
  sed -i "s/ quiet//g" /boot/uenv.txt
fi
sync
...
if main; then
  ...
  sleep 5
  whiptail --msgbox "Could not expand filesystem, something went wrong!!!\n${FAIL_REASON}" 20 60
fi
